---
id: Perso.projects.dendronlike. features.bookmarks
title: Perso.projects.dendronlike. features.bookmarks
desc: Bookmarks 
updated: 1780033919341
created: 0
---
### Add a bookmarks feature to dendronline. 

Still nipht.io will go off soon, I need a replacement.

Import initial data from nipht.io export.
Make use of open-graph to get image and description. Specifically :
 
  - og:description for a short description
  - og:image for an image
  - og:title for ... title
  - og:image:alt for image alt text

From notes (tree or stash) allow to pick a bookmark.

Storage at repo root in a single bookmarks.md mardown file.
The file should be easy to view in github.com.

A bookmarks is
 - a title
 - a link
 - a description 
 - a social network image (and alt text)
 - a list of tags

**To be read bookmarks**

bookmarks can be marked to be read. 
User can filter bookmarks on their "to be read" flag.
When a bookmark is added is automatically flagged as "to be read"

