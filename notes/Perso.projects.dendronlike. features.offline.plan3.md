...
#### [MODIFY] Backend & Sync Logic (3-Way Merge)
- **Automatic Resolution**: To resolve conflicts automatically without prompting the user, we will implement a 3-way text merging strategy for concurrent edits (e.g. Phone edits v1 -> v2.phone, Laptop edits v1 -> v2.laptop).
- We will integrate a lightweight 3-way text merge library (such as Google's `diff-match-patch`).
- When the offline sync pushes an edit to the server, the server (or client during sync) will check the base version.
- If a conflict is detected, it will apply the 3-way merge algorithm to seamlessly combine the changes into a merged v3 without requiring user intervention.
...
- **Automatic Conflict Resolution**: Simulate a conflict by editing a note on GitHub/Laptop while the phone is offline, edit the same note offline on the phone, go online, and verify the edits are automatically merged cleanly using the 3-way merge algorithm.
